...
def balance(m):
    """Compara la masa de liquidaciones ARRIBA vs ABAJO. Regla del train en vivo:
    si esta ~50/50 el precio no tiene un iman dominante y tiende a OSCILAR entre
    ambas zonas (rango); si esta descompensada, el lado pesado atrae al precio.
    Devuelve (etiqueta, pct_arriba, texto) o None."""
    if not m:
        return None
    w_up = sum(w for _, w in m["arriba"])
    w_dn = sum(w for _, w in m["abajo"])
    total = w_up + w_dn
    if total <= 0:
        return None
    pct_up = w_up / total * 100
    if 40 <= pct_up <= 60:
        et = "EQUILIBRADO"
        txt = ("liquidaciones ~50/50 (%.0f%% arriba / %.0f%% abajo): sin iman dominante, "
               "el precio tiende a OSCILAR entre ambas zonas -> esperar RANGO, no tendencia"
               % (pct_up, 100 - pct_up))
    elif pct_up > 60:
        et = "SESGO ARRIBA"
        txt = ("%.0f%% de la masa esta ARRIBA: mas combustible de squeeze alcista "
               "(el precio tiende a buscar las liq. de shorts)" % pct_up)
    else:
        et = "SESGO ABAJO"
        txt = ("%.0f%% de la masa esta ABAJO: mas combustible de cascada bajista "
               "(el precio tiende a buscar las liq. de longs)" % (100 - pct_up))
    return et, pct_up, txt
