...
def tamano(s, saldo):
    ...
    # Neto REAL del plan de salida: 50% en TP1 (1x dist) + 50% en TP2 (RR x dist),
    # no la posicion completa corriendo hasta TP2. Las fees no cambian: el volumen
    # total operado (apertura + cierre) es el mismo se reparta en una salida o dos.
    neto = notional * s["dist_pct"] / 100.0 * (1 + RR) / 2.0 - fees
    s.update({"riesgo_usd": riesgo_usd, "notional": notional, "lev": lev,
              "margen": margen, "neto": neto})
    return s
